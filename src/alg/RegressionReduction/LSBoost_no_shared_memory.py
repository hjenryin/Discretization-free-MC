...
# https://github.com/Declancharrison/Level-Set-Boosting/blob/main/LSBoost.py
# Shared Memory creates issues like failure to initialize shared memory,
# and it wouldn't be possible to run multiple experiment in parallel.
# Now that shared memory should only be readable, it might be a good idea 
# to just save them.

# SKlearn models have nogil implemented, so it's better to use threading to reduce loky overhead
...
class LSBoostingRegressor:
    ...
    '''
    def __init__(self, T = 100, num_bins = 10, min_group_size=1, global_gamma=.1, weak_learner = LinearRegression(), min_val=0, max_val=1, bin_type = 'default', initial_model = None, train_predictions = [], val_predictions = [], head_node_name ='head_node',  learning_rate = 1, center_mean = False, final_round = False,  n_jobs = 1,quiet=False):
        self.quiet = quiet
        self.worker_shared_memory={}
    ...
    def update(self):
        ...
        self.save_for_workers(closest_centers_train, f'closest_centers_train_{self.round_count}')
        ...
        result = Parallel(n_jobs=cores, backend="threading")(delayed(self.worker)(level_set, self.round_count) for level_set in occupied_level_sets)
        ...
        self.release_for_workers(f'closest_centers_train_{self.round_count}')
    ...
    def update_validation(self):
        ...
        self.save_for_workers(closest_centers_train, f'closest_centers_train_{self.round_count}')
        self.save_for_workers(closest_centers_val, f'closest_centers_val_{self.round_count}')
        
        # create snapshot for shared memory
        validation_predictions = copy.deepcopy(self.val_predictions)
        self.save_for_workers(validation_predictions, f'validation_predictions_{self.round_count}')
        ...
        # fit weak learners in parallel over non-empty level sets
        result = Parallel(n_jobs=cores, backend="threading")(delayed(self.worker_validation)(level_set, self.round_count) for level_set in occupied_level_sets)
        ...
        # release shared memory
        self.release_for_workers(f'closest_centers_train_{self.round_count}')
        self.release_for_workers(f'closest_centers_val_{self.round_count}')
        self.release_for_workers(f'validation_predictions_{self.round_count}')
    ...
    def predict(self, X):
        ...
        while True:
            ...
            if 'snapshotNode' == self.current_node.node_name:
                ...
                # parallel predict
                Parallel(n_jobs=cores, require="sharedmem",backend="threading")(delayed(update_predictions)(weak_learner[0], weak_learner[1].predict, X, closest_centers, weak_learners_predictions, lock) for weak_learner in self.current_node.weak_learners)
    ...
    def fit(self, x_train, y_train):
        ...
        # share training data
        self.save_for_workers(x_train,'x_train')
        self.save_for_workers(y_train,'y_train')
        ...
        iterator = tqdm(range(self.T), bar_format = '{l_bar}{bar} {n_fmt}/{total_fmt} {remaining}{postfix}', desc = 'LS', colour = 'green', leave = True, postfix={'Training Error':'{:.7f}'.format(self.train_list[-1])},disable=self.quiet)
    ...
    def fit_validation(self, x_train, y_train, x_val, y_val):
        ...
        self.save_for_workers(x_train,'x_train')
        self.save_for_workers(y_train,'y_train')
        self.save_for_workers(x_val,'x_val')
        self.save_for_workers(y_val,'y_val')
        ...
        if self.bin_type == 'distribution':
            ...
            # create level sets by equal distance
            self.discretized_endpoints = np.linspace(self.min_val, self.max_val, self.num_bins,endpoint=False)+(self.max_val-self.min_val)/self.num_bins/2
            self.discretized_points = self.discretized_endpoints
        ...
        # define iterator for progress bar
        iterator = tqdm(range(self.T), bar_format = '{l_bar}{bar} {n_fmt}/{total_fmt} {remaining}{postfix}', desc = 'LS', colour = 'green', leave = True, postfix={'Training Error':'{:.7f}'.format(self.train_list[-1]), 'Val Error':'{:.7f}'.format(self.val_list[-1]), 'Lowest Val Error': '{:.7f}'.format(lowest_val_error)},disable=self.quiet)
    ...
    def save_for_workers(self,data:np.ndarray,name):
        self.worker_shared_memory[name] = data.astype(np.float64)
        
    def release_for_workers(self,name):
        self.worker_shared_memory.pop(name)
...
### LSBoost PARALLEL FUNCTIONS ###
    ...
    def worker(self,index, counter):
        '''
        Parallel process worker. Fits weak learner on level set data, returns level set information
        '''
        shared_dict_readonly=copy.deepcopy(self.worker_shared_memory)
        # load shared memory array
        x_train_shared = shared_dict_readonly['x_train']
        y_train_shared = shared_dict_readonly['y_train']
        # load closest level set indices
        closest_centers_train = shared_dict_readonly[f'closest_centers_train_{counter}']
        ...
        if indices_train.sum() <= self.min_group_size:
            return None

        # copy weak learner from class 
        locals()['clf_intermediate'+str(index)] = copy.deepcopy(self.weak_learner)
    ...
    def worker_validation(self,index, counter):
        '''
        ...
        x_train_shared = shared_dict_readonly['x_train']
        y_train_shared = shared_dict_readonly['y_train']
        x_val_shared = shared_dict_readonly['x_val']
        y_val_shared = shared_dict_readonly['y_val']
        
        # load validation predictions of current model
        model_ls_predictions_val = shared_dict_readonly[f'validation_predictions_{counter}']
        
        # load closest level set indices
        closest_centers_train = shared_dict_readonly[f'closest_centers_train_{counter}']
        closest_centers_val = shared_dict_readonly[f'closest_centers_val_{counter}']
        ...
        update = (ls_hypothesis_error_val < (1-self.global_gamma)*ls_model_error_val)
